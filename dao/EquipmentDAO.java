...
public class EquipmentDAO extends DAO {
    ...
    public void updateEquipment(Equipment equ) throws Exception {
        Connection con = getConnection();
        PreparedStatement st = con.prepareStatement("UPDATE EQUIPMENT SET equi_name=?, equi_fee=?, max_use=?, min_use=?, fac_id=?, equi_note=?, rese_meth=? WHERE equi_id=?");

        st.setString(1, equ.getEquiName());
        st.setInt(2, equ.getEquiFee());
        st.setTime(3, equ.getMaxUse());
        st.setTime(4, equ.getMinUse());
        st.setString(5, equ.getFacId());
        st.setString(6, equ.getEquiNote());
        st.setInt(7, equ.getReseMeth());
        st.setString(8, equ.getEquiId());
